...
export class DeleteOrderDialogComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    try {
      let response = await axios.get("https://backoffice-ronda.herokuapp.com/api/auth/order/" + orderId);
  ...
  async deleteOrder() {
    ...
    let orderId = this.orderId; 
    console.log(orderId);
      await axios.delete("https://backoffice-ronda.herokuapp.com/api/auth/order/delete/" + orderId)
        .then(response => {
